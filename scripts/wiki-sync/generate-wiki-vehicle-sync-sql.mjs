...
async function fetchVehiclePage({ pageNumber, pageSize }) {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      'user-agent': 'StarCitizenFleetManager/0.1 catalog sync',
    },
  });
...
async function fetchVehiclePages({ pageSize, maxPages }) {
  const pages = [];
  let pageNumber = 1;
  let lastPage = 1;
...
async function main() {
  ...
  const batchSize = readPositiveInteger('WIKI_BATCH_SIZE', 10);
  const outputDir = resolve(process.env.WIKI_SYNC_SQL_DIR ?? 'supabase/generated/wiki-vehicles-sync');

  const pages = await fetchVehiclePages({ pageSize, maxPages });
  ...
  for (let index = 0; index < vehicles.length; index += batchSize) {
    ...
    const batchPayload = { data: batchVehicles };
    const sql = buildVehicleSyncSql({
      pages: [{ pageNumber: batchNumber, payload: batchPayload }],
    });
